...
def parse_args():
    ...
    parser.add_argument('-c', '--config_file', type=str, default='configs/debug.yml',
...
def train(args):
    ...
    train_dataloader_folds, valid_dataloader_folds, num_query_folds, num_classes_folds = get_loaders(cfg.DATASETS.DATA_PATH,
                                                                                  ...
                                                                                  shuffle_train=True)
    train_dl, val_dl, num_query, num_classes = train_dataloader_folds[0], valid_dataloader_folds[0], num_query_folds[0], num_classes_folds[0]
...
def test(args):
    ...
    cmc, mAP, all_AP = eval_func(distmat.numpy(), query_pid.numpy(), gallery_pid.numpy(), use_cython=True)
    ...
    cmc, mAP, all_AP = eval_func(distmat, query_pid.numpy(), gallery_pid.numpy(), use_cython=True)
